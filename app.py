...
from flask import Flask, request, redirect, url_for, session, render_template_string, abort, flash
...
def same_team_or_403(user_id: int):
    me = current_user()
    target = User.query.get_or_404(user_id)
    if me.role == "leader" and me.team_id == target.team_id:
        return
    if me.id == target.id:
        return
    abort(403)
...
def register():
    if request.method == "POST":
        ...
        role = "leader" if User.query.count() == 0 else "member"  # first user -> leader
    ...
    return render_template_string(TPL_REGISTER, **tplctx())
...
def login():
    if request.method == "POST":
        ...
        session["uid"] = user.id
        return redirect(url_for("dashboard"))
    return render_template_string(TPL_LOGIN, **tplctx())
...
def dashboard():
    ...
    return render_template_string(TPL_DASH, **tplctx(),
...
def team_dashboard():
    ...
    # Range filter
    rng = request.args.get("range","week")  # day|week|month
    ...
    for u in team_users:
        ...
        # accuracy: actual/estimate on completed tasks
    ...
    return render_template_string(TPL_TEAM, **tplctx(), me=me, members_stats=members_stats,
...
def finish_session():
    ...
    # update task actual_pomos if completed and >= planned
      ...
      <div class="card-body">
        ...
        elStart.addEventListener('click', async ()=>{
          if(!running){
            ...
            finishSession(sid, secs===0); // complete if time reached 0
...
# Allow templates in render_template_string
app.jinja_env.globals.update(TPL_BASE=TPL_BASE)
